    ...
    dbc.Row([
        ...
        dbc.Col([
            dbc.CardGroup([
                dbc.Card([
                    html.Legend("Saldo"),
                    html.H5("R$0", id='p-saldo-dashboards', style={})
...
def update_saldo(despesa, receita):
    df_despesas = pd.DataFrame(despesa)
    df_receitas = pd.DataFrame(receita)
...
def update_output(data_despesa, data_receita, despesa, receita):
    ...
    df_acum = df_ds.join(df_rc, how='outer')
    df_acum = df_acum.fillna(0)
